...
/**
 * Clase de adaptadores para convertir entre entidades y DTOs. Facilita la
 * comunicación entre capas sin exponer entidades de persistencia.
 */
public class Adaptadores {
     ...
     */
    public static MascotaArchivoDTO entidadADTO(Mascota mascota) {
        if (mascota == null) {
            return null;
        }

        MascotaArchivoDTO dto = new MascotaArchivoDTO();
    ...
    /**
     * Convierte un MascotaArchivoDTO a entidad Mascota.
     *
     * @param dto DTO de la capa de presentación
     * @return Entidad de dominio
     */
    public static Mascota dtoAEntidad(MascotaArchivoDTO dto) {
        if (dto == null) {
            return null;
        }
        ...
        if (dto.getId() != null && !dto.getId().isEmpty()) {
            try {
                ...
                // Si el ID no es válido, dejarlo null
                mascota.setId(null);
            }
        }
        ...
        mascota.setEstado(dto.getEstado() != null ? dto.getEstado() : "baja");
     ...
     */
    public static List<MascotaArchivoDTO> listEntidadADTO(List<Mascota> mascotas) {
        if (mascotas == null) {
            return new ArrayList<>();
        }

        List<MascotaArchivoDTO> dtos = new ArrayList<>();
     ...
     */
    public static List<Mascota> listDTOAEntidad(List<MascotaArchivoDTO> dtos) {
        if (dtos == null) {
            return new ArrayList<>();
        }

        List<Mascota> mascotas = new ArrayList<>();
        for (MascotaArchivoDTO dto : dtos) {
